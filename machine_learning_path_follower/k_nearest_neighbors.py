...
class ClassifyObject(Node):
    ...
    def _image_classifier(self,CompressedImage):
        ...
        # imgfolder = 'src/labfinal/labfinal/trained_model.sav/'
        # # imageDirectory = os.path.abspath(imgfolder)
        # # knn = cv2.ml.KNearest_create()
        # model = knn.load(os.path.abspath(imgfolder), 'rb')
        ...
        #     image_train = cv2.imread(imageDirectory + lines[i][0] + ".png")
        #     hsv_train = cv2.cvtColor(image_train, cv2.COLOR_BGR2HSV)
        #     mask1_train = cv2.inRange(hsv_train, (0, 90, 0), (180, 255, 255))
        #     mask_train = mask1_train
        #     contours_train, hierarchy = cv2.findContours(mask_train.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)

        #     if len(contours_train) != 0:
        #         cv2.drawContours(image_train, contours_train, -1, 255, 3)

        #         # find the biggest countour (c) by the area
        #         c = sorted(contours_train, key=cv2.contourArea, reverse=True)[:1]
        #         x, y, w, h = cv2.boundingRect(c[0])

        #         # draw the biggest contour (c) in green
        #         cv2.rectangle(image_train, (x, y), (x + w, y + h), (0, 255, 0), 2)
        #         crop_train = image_train[y:y + h, x:x + w]
        #         rect_area = w * h
        #         print('rect area: ', rect_area, ' | type: ', lines[i][1])

        #     data.append(crop_train)

        # # this line reads in all images listed in the file in GRAYSCALE, and resizes them to 33x25 pixels
        # train = np.array([np.array(cv2.resize(data[i], (40, 40))) for i in range(len(lines))])
